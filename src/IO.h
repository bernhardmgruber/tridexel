...
inline auto isFinite(vec3 v) -> bool {
	return std::isfinite(v.x) || std::isfinite(v.y) || std::isfinite(v.z);
}

inline auto isFinite(const Triangle& t) -> bool {
	return isFinite(t[0]) || isFinite(t[1]) || isFinite(t[2]);
}

inline void saveTriangles(std::filesystem::path path, const std::vector<Triangle>& triangles) {
...
inline void savePoints(std::filesystem::path path, const std::vector<vec3>& points) {
	if (path.has_parent_path())
		create_directories(path.parent_path());
	std::ofstream f{path, std::ios::binary};
	f << "ply\n";
	...
	f << "property float z\n";
	f << "end_header\n";
	f.write(reinterpret_cast<const char*>(points.data()), points.size() * sizeof(vec3));
	f.close();
}
